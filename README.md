...
- `anthropic`: LLM client used for structured extraction and synthesis.
- `biopython`: NCBI Entrez integration and PubMed XML parsing.
- `gradio`: Optional browser UI.
- `pydantic`: Runtime schema validation for LLM outputs.
...
- `ANTHROPIC_API_KEY`: Required for uncached LLM extraction or synthesis.
- `NCBI_API_KEY`: Optional but recommended for authenticated NCBI requests.
- `ANTHROPIC_MODEL`: Optional model override. Defaults to `claude-haiku-4-5`.

Important production note: configure a real Entrez email for NCBI compliance before production use. The current code sets `Entrez.email` inside `pipeline.py`; production deployments should make this configurable through `.env` or deployment secrets.
...
```powershell
python pipeline.py --query "ai drug discovery" --n 5 --output report.json
```

If uncached papers are processed, the command requires a valid Anthropic API key.
...
```powershell
python pipeline.py --query "BRCA1 PARP inhibitor resistance" --n 25 --output report.json
```

This performs PubMed retrieval, LLM extraction, validation, synthesis, and report serialization.
...
report = run_pipeline(
    query="single cell RNA sequencing melanoma immunotherapy",
    n=25,
    output="melanoma_immunotherapy_report.json",
)
...
### Fetch PubMed IDs only

```python
from pipeline import fetch_pubmed_ids

ids = fetch_pubmed_ids("protein folding neural network", n=10)
print(ids)
```

### Fetch and inspect PubMed metadata

```python
from pipeline import fetch_pubmed_ids, fetch_pubmed_records

ids = fetch_pubmed_ids("CRISPR base editing off target", n=10)
...
### Extract structured entities for one paper

```python
from pipeline import extract_entities, make_anthropic_client

paper = {
    "pmid": "example",
    "title": "Deep learning for protein structure prediction",
    "abstract": "This study uses transformer models and structural datasets...",
    "doi": "",
}
...
The app opens a local browser interface where users can enter a PubMed query, choose the number of papers, and inspect the structured JSON result.

## Engineering Notes for Production Hardening

Recommended next steps for larger deployments:

- Move Entrez email, request limits, batch sizes, cache directory, and output directory into environment-driven configuration.
- Add persistent raw PubMed XML/JSON storage for auditability and replay.
- Add deterministic unit tests for query normalization, DOI hashing, cache loading, schema validation, and output contracts.
- Add integration tests with mocked Entrez and mocked Anthropic responses.
- Add structured logging around request latency, retry behavior, cache hit rate, token usage, and model failures.
- Add explicit NCBI rate limiting across concurrent workers.
- Add a vector retrieval layer only when corpus size or synthesis requirements exceed direct-context prompting.

## License

Add the appropriate license for your intended distribution model before publishing this repository.
